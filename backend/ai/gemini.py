...
class GeminiClient:
    def __init__(self, api_key: str | None = None):
        ...
        if self.is_configured:
            try:
                # Set environment variable explicitly to ensure SDK picks up API key auth
    ...
    def _sanitize_sql(self, sql: str, timeframe: str) -> str:
        """Cleans up common invalid Exasol SQL patterns generated by LLM."""
        month_num = self._get_month_num(timeframe)
        
        # Replace illegal 'WHERE month = ...' clauses with valid Exasol function
        ...
        sql = sql.replace("order_amount", "total_amount")
        sql = sql.replace("shipping_days", "DATEDIFF(day, shipped_date, actual_delivery)")
    ...
    def _get_fallback_plan(self, query: str) -> Dict[str, Any]:
        """Generates query-tailored fallback plan using strictly valid Exasol SQL."""
        timeframe = self._extract_timeframe(query)
        month_num = self._get_month_num(timeframe)
            ...
            "hypotheses": [
                {
                    ...
                    "score": 78,
                    "signals": f"High concentration of drops in top SKU groups during {timeframe}",
                    "status": "leading"
                },
                {
                    "name": "Fulfillment & Delivery Delay Impact",
                    "description": "Check if shipping bottlenecks impacted sales conversion",
                    "sql": f"SELECT status, COUNT(*) AS shipment_count FROM shipments WHERE MONTH(shipped_date) = {month_num} GROUP BY status;",
                    "score": 32,
                    "signals": f"Shipping status remained within baseline during {timeframe}",
                    "status": "ruled_out"
                }
            ]
        }